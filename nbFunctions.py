    ...
    def fit(self, X, y):
        ...
        """for j in range(0,X.shape[1]):
            ...
            for i in range(0,X.shape[0]):
                if (Attr[i] in range(1,K)) and y[i]==1:
                    N1j+=1
            theta1[j]=(N1j+ alpha)/(N1+ K*alpha)"""
        ...
        """for j in range(0,X.shape[1]):
            ...
            k= np.unique(Attr).size
            for i in range(0,X.shape[0]):
                if (Attr[i] in range(1,k)) and y[i]==2:
                    N2j+=1
            theta2[j]=(N1j+ alpha)/(y.shape[0]- N1+ K*alpha)"""
        ##################
...
def evaluateBias(y_pred, y_sensitive):
    ...
    for i in range(0, y_pred.shape[0]):
        if y_sensitive[i]==2:
            count3+=1
        if y_pred[i] == 2 and y_sensitive[i] == 2:
            count1 += 1
